import {
  BASE_DAMAGE_DIVISOR, DEFAULT_BASE_HP, DRAIN_DISCOUNT_PER_CHARGE, HERO_POWER_DISTANCE_MOD_M, KEYWORDS, MATERIALS_PER_TURN,
...
- Some hulls carry CHARGE: pips that fill at the start of their owner's turn up to a printed max. "Discharge N" spends pips from that hull for an effect; an ability card that says "Discharge N from a friendly LH vehicle" spends them from one of yours. "Drain N Charge: costs Xk less" is a discount, never a requirement: when the pips across your whole board total at least N as you play the card, N of them are drained (taken first from hulls without a Discharge of their own) and it costs N × {{DRAIN_DISCOUNT_PER_CHARGE}} less; with fewer you pay the full printed price. Pips are public and die with the hull.
