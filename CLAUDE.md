...
planning policy and `BOT_FLOW=scored` plays the tempo evaluator with no
model; the default is the sectioned conversation. When a call fails or no
key is set, the evaluator plays, not the heuristic.
