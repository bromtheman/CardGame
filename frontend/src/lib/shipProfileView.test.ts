...
import { shipProfileHeadline, shipProfileRows } from './shipProfileView'
...
describe('shipProfileRows', () => {
  ...
  it('carries each row’s score, its reason, and a spoken form for the bar', () => {
    const { scores, matchups } = shipProfileRows(profile)
    expect(scores[0]).toEqual({
      key: 'firepower', label: 'Firepower', score: 5,
      spoken: 'Firepower 5 of 5', why: profile.scores.firepower.why,
    })
    expect(matchups[2]).toEqual({
      key: 'submarines', label: 'vs Submarines', score: 1,
