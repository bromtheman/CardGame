...
| `SET_DECK` | host or guest | Requires `deckId`; verifies `decks.owner_id = caller`. Writes the caller's `*_deck_id` and clears **the caller's own** ready flag. |
...
| `KICK` | host | Clears `guest_id`, `guest_deck_id`, `guest_ready`. |
| `LEAVE` | guest | As today, and additionally clears `guest_ready`. |
...
- The opponent's row shows their username and their deck's **faction**
  ...
  un-see it — plus their ready state.
...
- `scripts/smoke-lobby.mjs` — the new edge-function ops have no local
  ...
  asserting that each precondition rejects when unmet and that `games`
  gains a row.
