...
| A `zoneEffects` rider without `{ needsCatalog: true }` dies in production only | Recurring Threat carries it; Ambush, Ongoing Attrition and Sub Killer read no catalog and are safe to be skipped, and a runtime `CATALOG_EFFECTS` assertion pins the one that needs it |
